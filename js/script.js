$(function () {
	...
	//$rem_skills = clvl;
	//$("#output").find(".clvl").text(clvl).parent().find(".rem_skills").text($rem_skills);
	...
	$('.tree .tab > div').mousedown(function (e) {
		...
		if (e.which == 1) {
			//leftclick
			if (skill[$tab][$skill]['base'] < basemax && checkPreReq($skill) && hasRemaining($this)) {
			...
			//rightclick
			if (skill[$tab][$skill]['base'] > basemin && checkPreReqOf($skill)) {
				skill[$tab][$skill]['base'] -= 1;
				updateRemaining($this, 1);
				updateLevelRequired($this, -1);
				//$rem_skills += 1;
